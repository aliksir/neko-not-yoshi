...
- **重大度2層**: `block`（push 阻止 = exit 1）/ `warning`（報告のみ = exit 0）。allowlist は重大度とは別の抑制機構で、特定の検出結果を許可・降格する
...
### serve — Web UI
    ...
    node src/cli.mjs serve                 # ポート 7307 で Web UI を起動
    node src/cli.mjs serve --port 8080     # ポートを変更

NGワード / ホワイトリスト / allowlist をブラウザで閲覧・追加・削除できる Web UI を起動する。`127.0.0.1` のみにバインドするローカル専用（ネットワークに公開されない）。起動後に `http://localhost:7307` へアクセスする。
