...
- **2-tier severity**: `block` (prevents push = exit 1) / `warning` (report only = exit 0). The allowlist is a separate suppression mechanism from severity, allowing or downgrading specific detections
...
### serve — Web UI
    ...
    node src/cli.mjs serve                 # start Web UI on port 7307
    node src/cli.mjs serve --port 8080     # custom port

Launches a Web UI for browsing and managing NG words, whitelist, and allowlist entries from a browser. Binds to `127.0.0.1` only (localhost-only, not exposed to the network). Access `http://localhost:7307` after starting.
